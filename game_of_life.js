...
(function() {
  $(function() {
    var alive_color, calculateIfCellAlive, canvas_height, canvas_width, determineNextGenerationCellStates, generateWorld, num_columns, num_rows, paper, paused, populateWorld, runWorld, updateCells, updateWorld, world;
    num_rows = 30;
    num_columns = 48;
    canvas_width = 480;
    canvas_height = 300;
    ...
    generateWorld = function() {
      ...
      world.draw = function() {
        var c, r, _k, _results;
        _results = [];
        for (r = _k = 0; 0 <= num_rows ? _k < num_rows : _k > num_rows; r = 0 <= num_rows ? ++_k : --_k) {
          _results.push((function() {
            var _l, _results1;
            _results1 = [];
            for (c = _l = 0; 0 <= num_columns ? _l < num_columns : _l > num_columns; c = 0 <= num_columns ? ++_l : --_l) {
    ...
    calculateIfCellAlive = function(row, column) {
      ...
      countNeighbors = function() {
        var count, rel_positions;
        count = 0;
        rel_positions = [-1, 0, 1];
        rel_positions.forEach(function(row_position) {
          return rel_positions.forEach(function(column_position) {
            var cell_column, cell_row;
            if (!(row_position === 0 && column_position === 0)) {
              ...
              return count += (world[cell_row][cell_column].alive ? 1 : 0);
            }
          });
        });
        return count;
